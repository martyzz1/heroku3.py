...
_CLASS_ATTRS = {'arrays': {'array_of_subresources': SubResource},
                'bools': ['bool_value'],
                'dates': ['date_value'],
                'dicts': ['dict_value'],
                'ints': ['int_value'],
...
def test_to_python(str_value, date_value, int_value, dict_value, bool_value):
    ...
    Resource = type('Resource',
                    (BaseResource, ),
                    dict([('_{0}'.format(k), v) for k, v in _CLASS_ATTRS.items()]))
    data = {}
    for attr in itertools.chain(*_CLASS_ATTRS.values()):
        if attr in arguments and arguments[attr] is not None:
            data.update({attr: arguments[attr], })

    result = to_python(Resource(), data,
                       **dict([(k if k != 'map' else 'objects', v)
...
def test_to_python_raises_exception_on_invalid_date():
    Resource = type('Resource',
                    (BaseResource, ),
                    dict([('_{0}'.format(k), v) for k, v in _CLASS_ATTRS.items()]))
    data = {'date_value': '201-1-32 25:00:00.0+1:00', }

    with pytest.raises(ValueError):
        to_python(Resource(),
                  data,
                  **dict([(k if k != 'map' else 'objets', v)
                          for k, v in _CLASS_ATTRS.items()
                          ])
                  )
        ...
        for attr in attrs:
            ...
            if attr in data:
                if isinstance(attr_value, list):
                    assert all(isinstance(value, expected_type)
